...
const getCoastersByYear = (coasters, years) => {
    ...
    const coastersByYear = years.map(yearsRange => {
        ...
        let filter =  coasters.filter(eachCoaster => eachCoaster.year >= from && eachCoaster.year <= to)

        
        let integrated = filter.reduce(function(a,b) {
            return a.concat(b);
        });
        return integrated
    ...
    return coastersByYear.map(ele=>ele.length)
}
